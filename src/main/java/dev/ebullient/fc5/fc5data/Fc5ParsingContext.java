...
public class Fc5ParsingContext {
    ...
    public Fc5ParsingContext(String owner) {
        this.owner = owner;
    }

    private String getName() {
        if (name == null) {
            name = getOrDefault("name", owner);
        }
        return name;
    }
    ...
    public <T> T getOrDefault(String key, T defaultValue) {
        if (defaultValue instanceof Fc5Text) {
            List<String> list = getOrDefault(key, EMPTY_STRING_LIST);
            return (T) new Fc5Text(list);
        }
    ...
    public <T> T getOrFail(String parentElement, String key, Class<T> typeClass) {
        ...
        if (Fc5Text.class.isAssignableFrom(typeClass)) {
     ...
     */
    public static class FeatureParsingContext extends Fc5ParsingContext {
        boolean foundName;
