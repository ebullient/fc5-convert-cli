...
public class JsonIndex {
    ...
    private final Map<String, JsonNode> aliases = new HashMap<>();
    ...
    public JsonIndex(List<String> sources) {
        this.allowedSources.addAll(sources.stream().map(x -> x.toLowerCase()).collect(Collectors.toList()));
    ...
    public JsonNode getNode(CompendiumBase compendiumBase) {
        return nodeIndex.get(compendiumBase.sources.key);
    }
    ...
    JsonNode cloneOrCopy(String originKey, JsonNode value, IndexType parentType, String parentName, String parentSource) {
    ...
    private ArrayNode sortArrayNode(ArrayNode array) {
        ...
        array.forEach(x -> elements.add(x));
        ArrayNode sorted = Import5eTools.MAPPER.createArrayNode();
        elements.forEach(x -> sorted.add(x));
        return sorted;
    }
    ...
    private void doAddSpells(String originKey, JsonNode modItem, String modFieldName, JsonNode target) {
        ...
        List.of("constant", "will", "ritual").forEach(prop -> {
            ...
            modItem.get(prop).forEach(sp -> targetGroup.add(sp));
            ...
            case "appendIfNotExistsArr":
                if (tgtArray.size() == 0) {
                    ...
                    List<JsonNode> filtered = streamOf((ArrayNode) items)
                            .filter(it -> streamOf(tgtArray).noneMatch(x -> it.equals(x)))
                            .collect(Collectors.toList());
                    tgtArray.addAll(filtered);
                } else {
                    if (streamOf(tgtArray).noneMatch(x -> items.equals(x))) {
                        tgtArray.add(items);
                    }
                }
                break;
